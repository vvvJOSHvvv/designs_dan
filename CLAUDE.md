...
## 보트 카탈로그 (Marine 요트·상업용 보트) — 2026-09-25 시작, 자료 대기 중

사용자가 운영하는 **Marine**의 요트/상업용 보트 제작 카탈로그. 이 사이트(designs-dan)와는
별개 작업이다.

- **진행 순서**: PDF 카탈로그를 먼저 만들고, 나중에 웹에도 올릴 예정.
- **폴더 (사용자 Mac의 iCloud Drive 안, 사용자가 지정함)** — Mac 실제 경로는
  `~/Library/Mobile Documents/com~apple~CloudDocs/1. work/claude/boat`
  (Finder에서는 iCloud Drive > 1. work > claude > boat). iCloud의 "Mac 저장 공간 최적화"가
  켜져 있으면 파일이 클라우드에만 있고 로컬엔 자리표시만 있을 수 있으니, 작업 전에 폴더를
  우클릭 → "지금 다운로드"(또는 "다운로드 상태 유지")부터 할 것.
  - 작업 루트: `1. work/claude/boat`
  - 데이터(원본 사진·스펙 등 제작에 필요한 자료): `1. work/claude/boat/data`
  - 완성본 저장: `1. work/claude/boat/카탈로그`
- **주의**: 클라우드 세션(claude.ai/code)에서는 iCloud에 연결할 방법이 없어서 이 폴더에 직접 접근할 수 없다. 자료는 대화창 첨부나
  GitHub 업로드로 받고, 완성본은 사용자가 위 `카탈로그` 폴더에 저장하도록 전달한다.
  로컬(Mac) 세션이면 위 경로를 그대로 사용할 것.
- **데이터 원칙**: 모델 목록·스펙은 하나의 데이터 파일로 분리해서 관리 → 나중에 웹 버전에서
  그대로 재사용.
- **아직 받을 것**: 모델 목록(요트/상업용 구분, 한·영 이름), 모델별 사진, 스펙(길이·폭·흘수·
  엔진·속도·정원·탱크·선체재질), 소개 문구, 가격 표기 여부, 회사 정보·로고, 판형(A4 가로/세로),
  언어(한/영 병기·영문·한글).
